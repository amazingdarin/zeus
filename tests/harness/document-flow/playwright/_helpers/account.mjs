...
export function buildPreamble(context) {
  return [
    ...
    `  const payload = await page.evaluate(async ({ email, password }) => {`,
    ...
    `    });`,
    `    if (!response.ok) {`,
    `      throw new Error(\`programmatic login failed: ${'${response.status}'}\`);`,
    `    }`,
    `    return await response.json();`,
    `  }, { email, password });`,
    `  await page.evaluate((auth) => {`,
    `    window.localStorage.setItem('zeus_access_token', auth.access_token);`,
    `    window.localStorage.setItem('zeus_refresh_token', auth.refresh_token);`,
    `  }, payload);`,

...
import { Modal, Button, TreeSelect, Spin, Radio, Input } from "antd";
	...
	return items.map((item) => {
		...
		return {
			...
			children: item.children
				? convertToTreeSelectData(item.children, path)
				: undefined,
		};
	});
}
...
function DraftPreviewModal({
		...
		if (projectKey) {
			...
			fetchDocumentTree(projectKey)
				.then((tree) => {
					...
					setTreeData([{ value: "", title: "根目录" }, ...data]);
				})
				.catch((err) => {
	...
	// Check if can apply
	const canApply =
		!isEditMode || diffProgress.total === 0 || resolvedContent !== null;
	...
	const handleApply = useCallback(async () => {
		...
		try {
			...
			if (isSaveAsNew) {
				messageApi.success("文档副本已创建");
			} else {
				messageApi.success(result.isNew ? "文档已创建" : "文档已更新");
			}
			onApplied(result.docId, result.isNew);
			onClose();
		} catch (err) {
			messageApi.error(
				`应用草稿失败: ${err instanceof Error ? err.message : String(err)}`,
			);
		} finally {
			setApplying(false);
		}
	...
	const handleReject = useCallback(async () => {
		try {
			...
			messageApi.info("草稿已拒绝");
			onClose();
		} catch (err) {
			messageApi.error(
				`拒绝草稿失败: ${err instanceof Error ? err.message : String(err)}`,
			);
		}
	...
	return (
		<Modal
			title={
				<span>
					{isEditMode ? <EditOutlined /> : <FileTextOutlined />} {draft.title}

...
export default function CommandPalette() {
  ...
  const runCommand = async (commandId: string) => {
    ...
    try {
      const result = await executeCommand(commandId, {}, currentProject.projectRef, "palette");
              ...
              return (
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                    ...
                    <span style={{ color: "#888", fontSize: 12 }}>{command.pluginId}</span>
                  </div>
                  <div style={{ color: "#555", fontSize: 12, marginTop: 4 }}>{command.description}</div>
                  <div style={{ color: "#999", fontSize: 12, marginTop: 6 }}>
                    {(command.slashAliases || []).join(" ") || command.commandId}
                    {running ? " · 执行中..." : ""}
                  </div>
                </button>
              );

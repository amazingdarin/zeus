...
import { Switch, Spin, Collapse, Tooltip } from "antd";
...
function SkillsPanel() {
	...
	const handleFullAccessToggle = async (checked: boolean) => {
		try {
			...
			messageApi.success(checked ? "全权模式已开启" : "全权模式已关闭");
		} catch {
			messageApi.error("更新全权模式失败");
		} finally {
	...
	const handleToggle = async (skillId: string, enabled: boolean) => {
		...
		try {
			...
			messageApi.success(enabled ? "技能已启用" : "技能已禁用");
		} catch (err) {
				...
				prev.map((cat) =>
					cat.id === categoryId
						? {
								...cat,
								skills: cat.skills.map((skill) =>
									skill.isConfigurable ? { ...skill, enabled } : skill,
								),
							}
						: cat,
				),
	...
	return (
		<div className="skills-panel">
			<div className="skills-panel-header">
				<h3>AI 技能</h3>
				<p className="skills-panel-description">
					当前项目：{currentProject?.name || projectKey}
					。禁用的技能不会在对话中被触发，也不会出现在 / 命令列表。
				</p>
			</div>
			...
			<div className="skills-full-access">
				<div className="skills-full-access-info">
					...
					<div>
						<div className="skills-full-access-title">全权模式</div>
						<div className="skills-full-access-desc">
							开启后 AI 执行文档操作无需人工确认
						</div>
					</div>
				</div>
			...
			{categories.length === 0 ? (
				...
				>
					{categories.map((category) => {
						const configurableSkills = category.skills.filter(
							(s) => s.isConfigurable,
						);
						const requiredSkills = category.skills.filter(
							(s) => !s.isConfigurable,
						);
						const enabledConfigurable = configurableSkills.filter(
							(s) => s.enabled,
						).length;
						const allConfigurableEnabled =
							configurableSkills.length === 0 ||
							enabledConfigurable === configurableSkills.length;
						const someConfigurableEnabled =
							enabledConfigurable > 0 &&
							enabledConfigurable < configurableSkills.length;
							...
							>
								<div className="skills-list">
									{category.skills.map((skill) => (
										<div
											key={skill.id}
											className={`skill-item ${!skill.isConfigurable ? "skill-item-required" : ""}`}
										>
											<div className="skill-info">
												<span className="skill-command">

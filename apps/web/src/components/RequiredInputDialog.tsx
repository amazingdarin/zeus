...
import type {
	PendingRequiredInputInfo,
	ProvideRequiredInputPayload,
} from "../api/chat";
...
export default function RequiredInputDialog({
	...
	const requiredKeys = useMemo(() => {
		if (!pendingInput || pendingInput.kind !== "skill_args")
	...
	const submit = async () => {
		...
		try {
			...
			if (Object.keys(argsUpdate).length === 0) {
				messageApi.warning("请至少填写一个参数后继续，或点击取消操作。");
				return;
			}
	...
	const body = (
		<Space direction="vertical" size={10} style={{ width: "100%" }}>
			<div>
				<Space size={8}>
					<Tag color="gold">
						{pendingInput.kind === "doc_scope" ? "需要文档" : "需要参数"}
					</Tag>
			...
			{pendingInput.kind === "skill_args" && (
				<Form form={form} layout="vertical" style={{ width: "100%" }}>
					{Array.isArray(pendingInput.missing) &&
						pendingInput.missing.length > 0 && (
							<Text type="secondary">
								缺少参数: {pendingInput.missing.slice(0, 12).join(", ")}
							</Text>
						)}
					{Array.isArray(pendingInput.issues) &&
						pendingInput.issues.length > 0 && (
							<Paragraph
								type="secondary"
					...
					{pendingInput.fields.map((f) => {
						...
						const desc =
							f.description && f.description !== f.key ? f.description : "";
						...
						return (
							<Form.Item
								...
								extra={desc || undefined}
								rules={
									isRequired ? [{ required: true, message: "必填" }] : undefined
								}
							>
	...
	const actions = (
		<Space className="chat-inline-input-actions" size={8}>
			...
			<Button
				...
				onClick={submit}
				disabled={
					(pendingInput.kind === "doc_scope" && !selectedDocId) || loading
				}
				loading={loading}
			>
				继续执行

...
import { buildLegacyEditRedirect } from "../features/document-editor/new-document-routing";

// Resolution action type
type DiffResolution = "accept" | "reject";

function NewDocumentPage() {
	const { modalApi } = useAppFeedback();
	...
	const [baselineContent, setBaselineContent] = useState<JSONContent | null>(
		null,
	);
	...
	useEffect(() => {
		...
		navigate(
			buildLegacyEditRedirect({
				documentId: documentIdParam,
				parentId: parentIdParam,
			}),
			{ replace: true },
		);
	}, [documentIdParam, navigate, parentIdParam]);
	...
	const diffContentError = useMemo(() => {
		...
		return parsedJsonDraft ? null : "JSON 内容格式无效。";
	...
	const diffResult: BlockDiffResult = useMemo(() => {
		...
		try {
			...
			console.log(
				`[NewDocumentPage] blockDiff done, entries=${result.entries.length}`,
			);
			return result;
		} catch (err) {
		...
		async (id: string) => {
			...
			try {
				...
				if (metaValue) {
					...
					setParentID(
						String(metaValue.parent_id ?? metaValue.parent ?? "").trim(),
					);
					...
					if (
						...
						if (
							...
							Array.isArray((bodyValue as { content?: unknown }).content)
						) {
							contentToReturn = bodyValue as JSONContent;
							// Check for meta inside the wrapped format
							if (
								"meta" in bodyValue &&
								typeof (bodyValue as { meta?: unknown }).meta === "object"
							) {
								setContentMeta(
									(bodyValue as { meta?: EditorMeta }).meta as ContentMetaInput,
								);
							}
						}
						// Case 2: body is { type: "tiptap", content: { meta: ..., content: { type: "doc", ... } } }
							...
							if (bodyContent && typeof bodyContent === "object") {
								...
								if (
									"meta" in bodyContent &&
									typeof (bodyContent as { meta?: unknown }).meta === "object"
								) {
									setContentMeta(
										(bodyContent as { meta?: EditorMeta })
											.meta as ContentMetaInput,
									);
								}

								// Then handle wrapped { meta, content: { type: "doc", ... } } payloads.
								if (!contentToReturn && "content" in bodyContent) {
									const nestedContent = (bodyContent as { content?: unknown })
										.content;
									if (
										nestedContent &&
						...
						// Case 3: body has content field that might contain JSONContent
							...
							if (contentValue && typeof contentValue === "object") {
								...
								if (
									...
									setContentMeta(
										(contentValue as { meta?: EditorMeta })
											.meta as ContentMetaInput,
									);
								}
								// Try to extract nested content
								if ("content" in contentValue) {
									const nestedContent = (contentValue as { content?: unknown })
										.content;
									if (
										nestedContent &&
										typeof nestedContent === "object" &&
	...
	const handleSave = async () => {
		...
		try {
			...
			setSaveError(error instanceof Error ? error.message : "保存文档失败。");
		} finally {
			setSaving(false);
		}
	};
	...
	const applyResolvedDiffs = useCallback(() => {
		...
		const newContent: JSONContent = {
			type: "doc",
			content: newBlocks as JSONContent[],
		};
	...
	return (
		<div className="new-doc-page">
			...
			{diffContentError ? (
				<div className="doc-viewer-error">{diffContentError}</div>
			) : null}
			...
			<div className="new-doc-body">
				{diffMode ? (
							...
							{diffResult.entries.length === 0 ? (
								...
								diffResult.entries.map((entry, index) => {
									const originalDoc = entry.original
										...
										: null;
									const editedDoc = entry.edited
										? wrapBlockInDoc(entry.edited)
										: null;
									...
									return entry.status === "unchanged" ? (
										...
										>
											{originalDoc ? (
												...
												<div className="doc-viewer-state">暂无内容</div>
											)}
										</div>
									) : (
										<div
											...
											<div className="doc-diff-change">
												{entry.status === "modified" ? (
													...
													<>
														{originalDoc && (
															<div
																className={`doc-diff-change-item${resolution === "reject" ? " selected" : ""}`}
															>
																<div className="doc-diff-change-header">
																	<span className="doc-diff-change-title">
																		原始版本
																	</span>
																	{!isResolved && (
																		<button
																			...
																			type="button"
																			onClick={() =>
																				handleResolveDiff(index, "reject")
																			}
																		>
																			采用此版本
																		</button>
														...
														{editedDoc && (
															<div
																className={`doc-diff-change-item${resolution === "accept" ? " selected" : ""}`}
															>
																<div className="doc-diff-change-header">
																	<span className="doc-diff-change-title">
																		修改版本
																	...
																	{!isResolved && (
																		<button
																			...
																			type="button"
																			onClick={() =>
																				handleResolveDiff(index, "accept")
																			}
																		>
																			采用此版本
																		</button>
					...
					<div className="new-doc-json">
						<div className="new-doc-json-title">文档 JSON</div>
						<Input.TextArea
						...
						/>
						{jsonError ? (
							<div className="doc-viewer-error">{jsonError}</div>
						) : null}
					</div>
				) : (
